...
type ClothingItem = {
  id: string; name: string; category_id?: string | null; subcategory?: string | null; color?: string | null; size?: string | null; brand?: string | null; occasion?: string | null; season?: string | null; notes?: string | null; image_path?: string | null; is_favorite: boolean; usage_count?: number; last_used?: string | null; clothing_categories?: { id?: string; name?: string; is_active?: boolean } | null;
};
...
export default function WardrobeClient() {
  ...
  return <>
    ...
    <section className="section">{loading ? <div className="empty">Carregando…</div> : filtered.length ? <div className="grid">{filtered.map((item) => <article key={item.id} className="card clothing-card"><button aria-label={item.is_favorite ? 'Desfavoritar' : 'Favoritar'} className="favorite-btn" onClick={() => toggleFavorite(item)}><Heart size={17} fill={item.is_favorite ? 'currentColor' : 'none'} /></button><div className="clothing-image">{item.image_path ? <img src={`/api/media?path=${encodeURIComponent(item.image_path)}`} alt={item.name}/> : <div className="placeholder-art"><Shirt/></div>}</div><div className="clothing-info"><div className="clothing-name">{item.name}</div><div className="clothing-meta">{item.clothing_categories?.name || 'Sem categoria'}{item.color ? ` · ${item.color}` : ''}</div><div className="clothing-usage">{item.usage_count ? `${item.usage_count} ${item.usage_count === 1 ? 'uso' : 'usos'}` : 'Ainda não usada'}{item.last_used ? ` · ${new Date(`${item.last_used}T12:00:00`).toLocaleDateString('pt-BR',{day:'2-digit',month:'2-digit'})}` : ''}</div><div className="inline-actions" style={{marginTop:8}}><button className="btn btn-soft" onClick={() => setEditing(item)}>Editar</button><button className="btn btn-danger" onClick={() => setDeleting(item)}>Excluir</button></div></div></article>)}</div> : <div className="empty"><Shirt className="empty-icon"/><h3 className="empty-title">Seu guarda-roupa ainda está vazio.</h3><p className="empty-copy">Cadastre sua primeira peça com uma foto.</p><button className="btn btn-soft" onClick={startAdd}>Adicionar minha primeira peça</button></div>}</section>
