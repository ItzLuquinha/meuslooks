  ...
  return (
    <div className="outfit-editor">
      ...
      <section className="section compact-section">
        ...
        {selectedItems.length ? <div className="grid outfit-selected-grid">{selectedItems.map((item) => <article className="card clothing-card selected-piece" key={item.id}><button type="button" className="favorite-btn" aria-label={`Remover ${item.name}`} onClick={() => toggle(item.id)}><X size={16}/></button><div className="clothing-image">{item.image_path ? <img src={`/api/media?path=${encodeURIComponent(item.image_path)}`} alt={item.name} loading="lazy" decoding="async"/> : <div className="placeholder-art"><Shirt/></div>}</div><div className="clothing-info"><div className="clothing-name">{item.name}</div><div className="clothing-meta">{categoryName(item.clothing_categories)}</div></div></article>)}</div> : <div className="empty">Nenhuma peça selecionada ainda.</div>}
      </section>
      ...
      <section className="section compact-section">
        ...
        <div className="outfit-groups">
          {grouped.map((group) => <section className="outfit-group" key={group.key}><h4>{group.key}</h4><div className="grid">{group.items.map((item) => { const picked = selected.includes(item.id); return <button type="button" key={item.id} className={`card pick-card${picked ? ' selected' : ''}`} onClick={() => toggle(item.id)} aria-pressed={picked}>{picked && <span className="pick-selection-mark" aria-hidden="true">✓</span>}<div className="clothing-image">{item.image_path ? <img src={`/api/media?path=${encodeURIComponent(item.image_path)}`} alt="" loading="lazy" decoding="async"/> : <div className="placeholder-art"><Shirt/></div>}</div><div className="clothing-info"><div className="clothing-name">{item.name}</div><div className="clothing-meta">{categoryName(item.clothing_categories)} · {picked ? 'Selecionada' : 'Selecionar'}</div></div></button>; })}</div></section>)}
        </div>
      </section>
