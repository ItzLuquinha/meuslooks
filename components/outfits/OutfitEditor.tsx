    ...
    <div className="outfit-editor">
      ...
      <section className="section compact-section">
        ...
        <div className="outfit-groups">
          {grouped.map((group) => <section className="outfit-group" key={group.key}><h4>{group.key}</h4><div className="grid">{group.items.map((item) => { const picked = selected.includes(item.id); return <button type="button" key={item.id} className={`card pick-card${picked ? ' selected' : ''}`} onClick={() => toggle(item.id)} aria-pressed={picked}><div className="clothing-image">{item.image_path ? <img src={`/api/media?path=${encodeURIComponent(item.image_path)}`} alt=""/> : <div className="placeholder-art"><Shirt/></div>}</div><div className="clothing-info"><div className="clothing-name">{item.name}</div><div className="clothing-meta">{categoryName(item.clothing_categories)} · {picked ? 'Selecionada' : 'Selecionar'}</div></div></button>; })}</div></section>)}
        </div>
      </section>

      <label className="check-row"><input type="checkbox" checked={favorite} onChange={(event) => setFavorite(event.target.checked)}/><Heart size={17}/> Favoritar look</label>
