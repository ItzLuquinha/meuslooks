...
export async function PATCH(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (isDayLook) {
    const { data: previousDay } = await admin.from('outfits').select('id').eq('user_id', user.id).eq('is_day_look', true).neq('id', id);
    previousDayIds = (previousDay || []).map((entry: { id: string }) => entry.id);
    const { error: clearDayError } = await admin.from('outfits').update({ is_day_look: false }).eq('user_id', user.id).neq('id', id);
