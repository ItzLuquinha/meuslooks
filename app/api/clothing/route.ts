...
export async function POST(req: Request) {
  ...
  if (categoryName) {
    const { data: category } = await admin.from('clothing_categories').select('id').eq('name', categoryName).eq('is_active', true).or(`user_id.is.null,user_id.eq.${context.userId}`).limit(1).maybeSingle();
  ...
  const { data: item, error } = await admin.from('clothing_items').insert({
    id, user_id: context.userId, name, category_id: categoryId,
